...
int main(void) {
	/*
	* When does a memory goes to the heap area? By using malloc(), calloc(), valloc(), ..., new only.
	* This allocated memory MUST be released by your hand, otherwise a memory leak still exists while the
	* system is running.
	*
	* When does a memory goes to the stack area? Usually, at almost each time you're using any variable.
	* Even a pointer may refer to a stack block. Since a memory has not been allocated before, this
	* will never be moved to the heap area.
	*/

	int x = 100;                                              // stack
	int array[] = {1,2,3,4,5};                                // stack
	int *ptr = (int *) calloc(5, sizeof(int));                // heap
	int *help = ptr;                                          // heap, because this is a shallow copy of *ptr
	struct S s0;                                              // stack
	struct S *s1 = (struct S*) malloc(sizeof(struct S));      // heap
	struct S *s2 = NULL;                                      // usually stack, but this causes an error output on runtime
