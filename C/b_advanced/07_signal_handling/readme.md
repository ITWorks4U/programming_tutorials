...
>   **NOTE**: A signal can also be raised by your hand. Use the raise() function, whereas a signal has been registered before.

### different kind of signals

>   **NOTE**: *The given signal ID's are **NOT** identical on each system. So a signal ID on system A may differ to a signal ID on system B!*
...
| SIGABRT   | 6 |  abnormal termination of your process; this is not the next level of SIGTERM   | `assert(condition)` results to false, call `abort()` function, ...           | every system |
...
| SIGPIPE   | 13 | Broken pipe | Sending data trough an already closed pipe | on most POSIX / LINUX systems |
...
| SIGTTOU   | 22 |  Background process tried to write to the terminal | A background job writes tho the terminal without permission | UNIX (most Linux systems)
