...
/*  often used: a hashed map  */

typedef void (*command_handler_t)(void);          //  any handler function of type void without any arguments

typedef struct {
  const uint32_t command;                         //  the hashed value
  command_handler_t handler;                      //  responding handler function
} CommandHandler;
