...
def main() -> None:
	...
	client.connect(host=cfg_reader.host_names[2], port=int(cfg_reader.ports[0]))

	# register a callback function => optional
	client.on_publish = on_publish

	command_position: int = 0
	commands: list[str] = ["green_on", "green_off", "yellow_on", "yellow_off", "red_on", "red_off"]

	while True:
		# NOTE: send data to /led/control channel
		client.publish(cfg_reader.publisher_channels[1], payload=commands[command_position])
		command_position += 1
