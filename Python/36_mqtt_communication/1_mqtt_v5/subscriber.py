...
def on_subscribe(client, userdata, mid, reason_code_list, properties) -> None:
	subscription = {
		...
		"userdata" : userdata,
		"message id" : mid,
		"reason_code_list " : reason_code_list,
		"properties" : properties
	}

	print(subscription)

	first_value = reason_code_list[0]

	if first_value.is_failure:
		print(f"broker rejected your subscription attempt: {first_value}")
	else:
		print(f"broker granted you the following subscription with QoS: {first_value.value}")
	#end if
#end function
...
def main() -> None:
	...
	client.connect(host=_cfg_reader.host_names[0], port=int(_cfg_reader.ports[0]))
